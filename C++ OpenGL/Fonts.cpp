...
void Fonts::Initialize(string _name, FT_UInt _size) {
	...
	// Iniitalize the library and font face
...
void Fonts::RenderText(std::string _text, float _x, float _y, float _scale, glm::vec3 _color) {
	...
	for (c = _text.begin(); c != _text.end(); c++) {
		...
		_x += (ch.Advance >> 6) * _scale; // Bitshift by 6 to get value in pixels (2^6 = 64 (divide amount of pixels by 64 to get amount of pixels))
	}
